...
class Task(Artifact):
    ...
    def read(self, task_key, encoding='utf-8'):
        list_dict_node_values = [{r'arr:string': task_key}]
        soap_read_action, soap_payload = self.payload_gen.get_soap_details('task', 'read', list_dict_node_values)
    ...
    def construct_verify_dict(self, encoding):
        if self.verify_update_key:
            ...
            for key in self.snap_shot_dct:
                node_value = key.split('ns1:')[-1]
                node_value = 'd:' + node_value
